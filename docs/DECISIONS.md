...
16. **Multi-source in one `.chrono/`** *(the real payoff of the general core)*. The `sources` table is populated per source, each with its own watermark; `chrono add <path>` ingests an extra source without wiping the index; `sync` iterates every source and reindexes only the one that diverged (rebase/force-push, rotated log). `source_id = <kind>:<abs path>` — the same value events carry, so `correlate` and per-source queries line up. **Git parity is preserved** (verified: rustworkx hotspots top unchanged; init+sync idempotent). Known limit: 2+ git repos in one index would fight over the file-`deleted` flag (entities are global, deduped by key); the supported shape is 1 git + N log sources, whose entities aren't `type='file'`.
